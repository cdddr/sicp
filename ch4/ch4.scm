...
;;; Only self-evaluting items are numbers and strings
(define (self-evaluating? exp)
  (cond ((number? exp) true)
        ((string? exp) true)
        (else false)))
...
;;; Definitions have the from (define <var> <value>) or (define (<var <params>) <body>)
(define (definition? exp)
  (tagged-list? exp 'define))
(define (definition-variable exp)
  (if (symbol? (cadr exp))
...
;;; A procedure application is any compound expression that is not one of the above types
